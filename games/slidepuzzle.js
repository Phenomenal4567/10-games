/* ARCADE10 — Slide Puzzle */
(function () {
  let container, root, tiles, blankIdx, moves, size, gameActive, timerInt, elapsed;

  function start_slidepuzzle(cont) {
    container = cont;
    showSizeSelect();
  }
  function stop_slidepuzzle() {
  ...
  function showSizeSelect() {
    if (!container) return;
    container.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%;background:#0a0a0f;gap:16px;padding:24px;box-sizing:border-box;">
        <div style="font:700 22px/1 'Bebas Neue',sans-serif;color:#fff;letter-spacing:3px;">SLIDE PUZZLE</div>
        <div style="font:400 13px/1.5 'DM Sans',sans-serif;color:#888;text-align:center;">Slide tiles to arrange them in order.<br>Top-left = 1, bottom-right = blank.</div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;justify-content:center;">
          ${[[3,'Easy'],[4,'Medium'],[5,'Hard']].map(([n,label])=>`
            <button data-n="${n}" style="
              background:linear-gradient(135deg,#1a1a2e,#16213e);border:1.5px solid #333;border-radius:12px;
              padding:18px 24px;cursor:pointer;color:#fff;font-family:inherit;min-width:90px;
              transition:border-color .2s,transform .15s;">
              <div style="font:700 22px/1 'Bebas Neue',sans-serif;color:#a259ff;">${n}×${n}</div>
              <div style="font:400 11px/1.4 'DM Sans',sans-serif;color:#888;margin-top:4px;">${label}</div>
              <div style="font:400 10px 'DM Mono',monospace;color:#555;margin-top:2px;">
                BEST: ${Store.get(`arcade_slide_best_${n}x${n}`)||'—'} moves
              </div>
            </button>
          `).join('')}
        </div>
  ...
  function startGame(n) {
    ...
    for (let i = 0; i < 1000; i++) {
      const neighbors = getNeighbors(blankIdx, n);
      const swap = neighbors[Math.floor(Math.random()*neighbors.length)];
      [tiles[blankIdx], tiles[swap]] = [tiles[swap], tiles[blankIdx]];
      blankIdx = swap;
    }
    buildUI();
    clearInterval(timerInt);
    timerInt = setInterval(()=>{elapsed++;updateHUD();},1000);
  ...
  function buildUI() {
    if (!container) return;
    const cellSize = Math.min(Math.floor((Math.min(container.clientWidth, container.clientHeight) - 80) / size), 90);
    container.innerHTML = `
      <div id="sp-root" style="display:flex;flex-direction:column;align-items:center;width:100%;height:100%;background:#0a0a0f;padding:12px;box-sizing:border-box;gap:10px;position:relative;">
        <div style="display:flex;justify-content:space-between;width:100%;max-width:${size*cellSize+20}px;">
          <div style="text-align:center;">
            <div style="font:500 10px/1 'DM Mono',monospace;color:#ffffff44;">MOVES</div>
            <div id="sp-moves" style="font:700 18px/1 'Bebas Neue',sans-serif;color:#fff;">0</div>
          </div>
          <div style="text-align:center;">
            <div style="font:500 10px/1 'DM Mono',monospace;color:#ffffff44;">TIME</div>
            <div id="sp-time" style="font:700 18px/1 'Bebas Neue',sans-serif;color:#4cc9f0;">0s</div>
          </div>
          <div style="text-align:center;">
            <div style="font:500 10px/1 'DM Mono',monospace;color:#ffffff44;">BEST</div>
            <div style="font:700 18px/1 'Bebas Neue',sans-serif;color:#ffd60a;">${Store.get(`arcade_slide_best_${size}x${size}`)||'—'}</div>
          </div>
        </div>
        <div id="sp-grid" style="display:grid;grid-template-columns:repeat(${size},${cellSize}px);grid-template-rows:repeat(${size},${cellSize}px);gap:4px;"></div>
        <button id="sp-back" style="background:#ffffff0d;border:1.5px solid #ffffff22;color:#aaa;padding:8px 20px;border-radius:8px;font:700 13px/1 'Bebas Neue',sans-serif;cursor:pointer;letter-spacing:1px;">CHANGE SIZE</button>
      </div>`;
  ...
  function renderGrid(cellSize) {
    if (!container) return;
    const cs = cellSize || Math.min(Math.floor((Math.min(container.clientWidth,container.clientHeight)-80)/size),90);
    ...
    tiles.forEach((val,idx) => {
      ...
      el.style.cssText = `
        ...
        font:700 ${cs*0.35}px/1 'Bebas Neue',sans-serif;
        cursor:${val===0?'default':'pointer'};
        transition:background .15s,box-shadow .15s;
        ${val===0?'background:#1a1a2e;border:2px dashed #333;':'background:linear-gradient(135deg,#a259ff,#7c3aed);color:#fff;box-shadow:0 2px 8px #a259ff44;'}
      `;
      el.textContent = val === 0 ? '' : val;
      el.dataset.idx = idx;
      if (val !== 0) el.addEventListener('click', () => tryMove(idx));
      grid.appendChild(el);
    });
  }
  ...
  function finishGame() {
    ...
    overlay.innerHTML=`
      ...
      <div style="font:400 13px/1.6 'DM Mono',monospace;color:#aaa;text-align:center;">
        ...
        ${isNew?'<span style="color:#ffd60a;">★ NEW BEST!</span>':`Best: ${Store.get(key)} moves`}
      </div>
